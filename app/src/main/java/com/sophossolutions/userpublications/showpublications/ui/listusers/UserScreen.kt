...
fun UsersScreen(viewModelUser: ViewModelUser, navController: NavController) {
    ...
    viewModelUser.onClearSearchBar()

    val isLoading by viewModelUser.isLoadingUsers.collectAsState()
...
fun SearchBarUser(viewModelUser: ViewModelUser, navController: NavController) {
    ...
    val nameUser by viewModelUser.nameUser.collectAsState()
    ...
    Column(
        ...
        SearchBar(
            ...
            //TODO: Guardar el historial de búsqueda
            //TODO: El texto buscado debe aparecer en pantalla
...
fun UsersFilter(viewModelUser: ViewModelUser, navController: NavController) {
    ...
    LazyColumn {
        items(
            viewModelUser.onSearchUser(viewModelUser.nameUser.value).toList()
        ) {
            Card(
                modifier = Modifier
                    .fillMaxWidth()
                    ...
                    .clickable {
                        navController.navigate(
                            AppRoutesScreen.ScreenUserPublications.route.plus("/${it.first}")
                        )
                    },
